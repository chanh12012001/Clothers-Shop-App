...
import com.myclothershopapp.adapter.MyOrderItemAdapter;
...
public class OrderDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                                        ...
                                        case "Cancelled":
                                            ...
                                            if (model.getPackedDate().after(model.getOrderedDate())){
                                                ...
                                                if (model.getShippedDate().after(model.getPackedDate())){
                                                    ...
                                                    deliveredBody.setText("Đơn hàng của bạn đã bị hủy.");
                                                    ...
                                                    shippedBody.setText("Đơn hàng của bạn đã bị hủy.");
                                                ...
                                                packedBody.setText("Đơn hàng của bạn đã bị hủy.");
                                                                                        ...
                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                            ...
                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                if (task.isSuccessful()){
                                                                                                    ...
                                                                                                    FirebaseFirestore.getInstance().collection("ORDERS").document(model.getOrderID()).update("Order Status", "Cancelled");

                                                                                                    cancelOrderBtn.setTextColor(getResources().getColor(R.color.unsuccessred));
                                                                                                    cancelOrderBtn.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#FFFFFF")));
                                                                                                }else {
                                                                                                    String error = task.getException().getMessage();
                                                                                                    DynamicToast.make(OrderDetailsActivity.this, error, Toast.LENGTH_SHORT).show();
                                                                                                }
                                                                                                loadingDialog.dismiss();
                                                                                            }
                                                                                        });
        ...
        totalItems.setText("Giá ("+totalitem+" món hàng)");
        totalAmount.setText(totalammount);

        totalItemsPrice.setText(totalitemprice +" đồng");
        if (deliverypricee.equals("FREE")){
            deliveryPrice.setText("FREE");

        }else {
            deliveryPrice.setText(deliverypricee + " đồng");
        }
        savedAmount.setVisibility(View.GONE);

              savedAmount.setText("Bạn đã tiết kiệm"+ savedammount +" đồng cho đơn hàng này.");
    }
