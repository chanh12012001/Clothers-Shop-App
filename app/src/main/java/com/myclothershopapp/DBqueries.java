...
import com.myclothershopapp.activities.AddAddressActivity;
import com.myclothershopapp.activities.DeliveryActivity;
import com.myclothershopapp.activities.NotificationActivity;
...
import com.myclothershopapp.adapter.MyOrderAdapter;
...
import com.myclothershopapp.model.AddressesModel;
...
import com.myclothershopapp.model.MyOrderItemModel;
import com.myclothershopapp.model.MyOrderMainModel;
import com.myclothershopapp.model.NotificationModel;
...
public class DBqueries {
                ...
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            DynamicToast.make(context, error, Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }

    public static void loadFragmentData(final RecyclerView homePageRecyclerView, final Context context, final int index, String categoryNames, FragmentActivity activity) {
        ...
        firebaseFirestore.collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_WISHLIST")
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    ProductDetailsActivity.ALREADY_ADDED_TO_WISHLIST = false;
                    DynamicToast.make(context, "Sản phẩm đã xóa khỏi danh sách yêu thích .", Toast.LENGTH_SHORT).show();
    ...
    public static void removeFromCart(final String pid, final int index, final Context context, final TextView cartTotalAmount) {
        ...
        updateCartList.put("list_size", (long) cartList.size());
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    DynamicToast.make(context, "Sản phẩm bị xóa khỏi giỏ hàng .", Toast.LENGTH_SHORT).show();
                                        ...
                                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                            if (task.isSuccessful()) {
                                                for (QueryDocumentSnapshot documentSnapshot : task.getResult()) {
                                                    ...
                                                    } else if (documentSnapshot.get("type").toString().equals("Thẻ giảm giá") && lastseenDate.before(documentSnapshot.getDate("validity"))) {
                                                        rewardModelList.add(new RewardModel(documentSnapshot.getId(), documentSnapshot.get("type").toString()
                                                                , documentSnapshot.get("lower_limit").toString()
                                                                , documentSnapshot.get("upper_limit").toString()
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (task.getResult().isEmpty()) {
                                loadingDialog.dismiss();
                                DynamicToast.make(context, "Bạn chưa đặt hàng", Toast.LENGTH_SHORT).show();
                            } else {
    ...
    public static int getproductPrice(Context context,String price){
    ...
    public static void setCuttedPrice(View view,Context context,String price){
        PrefManager prefManager = new PrefManager(context);
        if (prefManager.getDiscountAvailable()){
